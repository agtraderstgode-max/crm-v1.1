    ...
    <div className={cn(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-slate-800">Letter Pad Editor</h1>
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 border border-emerald-200">
                <Sparkles className="h-3 w-3" /> AG TRADERS Official
              </span>
            </div>
            <p className="text-xs text-slate-500">
              அலுவலக கடிதங்கள், கொட்டேஷன் மற்றும் நோட்டீஸ் தயார் செய்து அச்சிடவும்
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handlePrint}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-semibold shadow-xs transition-colors"
      ...
      <div className={cn(
        ...
        isFullscreen ? "flex-1 h-full" : "h-[calc(100vh-185px)] min-h-[600px]"
      )}>
        <iframe
          key={iframeKey}
          ref={iframeRef}
      ...
      {/* Helpful Footer hint when not fullscreen */}
      {!isFullscreen && (
        <div className="flex items-center justify-between text-[11px] text-slate-400 px-1">
          <span className="flex items-center gap-1.5">
            <Info className="h-3.5 w-3.5 text-slate-400" />
            தானியங்கி சேமிப்பு வசதி உள்ளது (Auto-saves continuously in your browser).
          </span>
          <span>Tip: Press <strong>Cmd+B</strong> for Bold, <strong>Cmd+I</strong> for Italic inside the sheet.</span>
        </div>
      )}
    </div>
  )
